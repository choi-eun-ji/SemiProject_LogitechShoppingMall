...
public class MemberDAO implements InterMemberDAO {
	...
	// 결제 테이블 입력
	@Override
	public int purchaseUpdate(Map<String, String> paraMap) throws SQLException {
		int n = 0;
		int n1 = 0;
		int n2 = 0;
		int n3 = 0;
		...
		try {
			...
			String sql = "insert into purchase(purchaseno, fk_memberno, receiver, postcode, address, detailaddress, extraaddress, payment, totalprice, ordernum)\n"+
					"values(purchase_seq.nextval, ?, ?, ?, ?, ?, ?, 'card', ?, purchase_seq.nextval||'-'||to_char(sysdate, 'yymmdd'))";

			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, paraMap.get("fk_memberno"));
			pstmt.setString(2, paraMap.get("receiver"));
			pstmt.setString(3, paraMap.get("postcode"));
			pstmt.setString(4, paraMap.get("address"));
			pstmt.setString(5, paraMap.get("detailaddress"));
			pstmt.setString(6, paraMap.get("extraaddress"));
			pstmt.setString(7, paraMap.get("totalprice"));
			...
			if(result == 1) {
				...
				sql = "select purchase_seq.CURRVAL as fk_purchaseno\n"+
					"from dual;";
				...
				String fk_purchaseno = "";
				
				if(rs.next()) {
					fk_purchaseno = rs.getString(1);
				}
				
				// 포인트 누적
				sql = "insert into tbl_point(pointnum, reason, fk_purchaseno)\n" +
						"values(?, '구매에 대한 적립금', ?)";
				
				pstmt = conn.prepareStatement(sql);
				pstmt.setInt(1, (int)(Integer.parseInt(paraMap.get("totalprice"))*0.01));
				pstmt.setString(2, fk_purchaseno);
				
				n1 = pstmt.executeUpdate();
				
				// 포인트 사용
				if(paraMap.get("point") != null) {
					sql = "update member set point = point + ?\n" +
							"where memberno = ?";
				...
				// 한달 구매액에 해당하는 등급
				sql = "select membershipname\n"+
						"from\n"+
						"(\n"+
						"select sum(totalprice) AS monthsum\n"+
						"from purchase\n"+
						"where fk_memberno = ?\n"+
						"and months_between(sysdate, purchaseday) <= 1\n"+
						") V, membership M\n"+
						"where monthsum between minstandard and maxstandard";
				...
				if(rs.next()) {
					membershipname = rs.getString(1);
				}
				
				// 회원등급
				sql = "update member set fk_membershipname = ?\n" +
						"where memberno = ?";
				
				pstmt = conn.prepareStatement(sql);
				pstmt.setString(1, membershipname);
				pstmt.setString(2, paraMap.get("fk_memberno"));
	...
	@Override
	public String getMemberPoint(String memberno) throws SQLException {
		String point = "";
