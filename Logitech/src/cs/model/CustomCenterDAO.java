...
public class CustomCenterDAO implements InterCutomCenterDAO {
	...
	// 페이징처리를 위해서 전체 faq에 대한 총페이지 개수 알아오기(select) 
		...
		@Override
		public OneQueryAnswerVO getAnswervo(String seqoq) throws SQLException {
			...
			OneQueryAnswerVO oqavo= null;
				...
				try {
					...
					pstmt = conn.prepareStatement(sql);
					pstmt.setString(1, seqoq);
					rs=pstmt.executeQuery();
					
					if(rs.next()) {
						oqavo= new OneQueryAnswerVO();
						oqavo.setSeq_oq(rs.getString(1));
						oqavo.setOq_content(rs.getString(2));
						oqavo.setAnswerdate(rs.getString(3));
