...
@WebServlet(
		description = "사용자가 웹에서 *.sg 을 했을 경우 이 서블릿이 먼저 응답을 해주도록 한다.", 
		urlPatterns = { "*.sg" }, 
		initParams = { 
				@WebInitParam(name = "propertyConfig", value = "C:/NCS/workspace(jsp)/Logitech/WebContent/WEB-INF/Command.properties", description = "*.sg 에 대한 클래스의 매핑파일")
		})
public class FrontController extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		/*
			...
			그러므로 init(ServletConfig config) 메소드는 FrontController 서블릿이 동작해야 할 환경설정을 하는데 사용한다.
		*/
		// *** 확인용 *** //
	//	System.out.println("~~~ 확인용 => 서블릿 FrontController 의  init(ServletConfig config) 메소드가 실행됨.");
	    ...
	    // key는 중복을 허락하지 않는다(중복값이 들어오면 덮어씌운다). value 값을 얻어오기 위해서는 key값만 알면 된다.
		
		FileInputStream fis = null;
		// 특정 파일에 있는 내용을 읽어오기 위한 용도로 쓰는 객체
		
		try {
			
			String props = config.getInitParameter("propertyConfig");
			// 파라미터의 ServletConfig config인 서블릿 환경변수.getInitParameter("initParam의 name") 메소드를 통해 value 값을 알아온다.
		//	System.out.println("~~~ 확인용 props => " + props);
			...
			pr.load(fis);
		/*
  			pr.load(fis); 는 fis 객체를 사용하여 C:/NCS/workspace(jsp)/MyMVC/WebContent/WEB-INF/Command.properties 파일의 내용을 읽어서
  			Properties 클래스의 객체인 pr에 로드하는 것이다.
		...
		*/
	        ...
	        */
				...
				*/
					cmdMap.put(key, obj);
					// init 메소드는 한 번만 실행되기 때문에 객체(인스턴스)를 map에 담아 필요할 때 사용할 수 있도록 한다.
			...
			System.out.println(">>> C:/NCS/workspace(jsp)/Logitech/WebContent/WEB-INF/Command.properties 파일이 없습니다. <<<");
			e.printStackTrace();
		} catch(IOException e) {
			e.printStackTrace();
		} catch(Exception e) {
	...
	//	System.out.println("~~~ 확인용 uri => " + uri);
		...
		AbstractController action = (AbstractController)cmdMap.get(key);  // 공통인 부모 클래스로 형변환해 받는다.
		
		if(action == null) {
			System.out.println(">>> " + key + " URL 패턴에 매핑된 클래스는 없습니다. <<<");
			// >>> /abc.up URL 패턴에 매핑된 클래스는 없습니다. <<<
		} else {
			try {
				action.execute(request, response);
				// @@@ 확인용 IndexController 의 인스턴스 메소드 execute 호출함 @@@
				// ### 확인용 MainController 의 인스턴스 메소드 execute 호출함 ###
