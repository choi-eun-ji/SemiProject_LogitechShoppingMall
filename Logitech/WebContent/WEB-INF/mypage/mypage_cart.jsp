   ...
   function selectOrder() {
	   var seqcart = "";
	      ...
	      var arrstr = "";
	  	  arrcheck.each(function(index, item){
	         if($("#check" + index).is(":checked")==true){
	        	 arrstr += $("#serialid"+index).val() + ",";
	         }
	  	  });
	  	alert(arrstr);
	  	
	  location.href = "<%= ctxPath%>/product/purchasedetail.sg?arrstr="+arrstr;
   }
</script>
...
<div class="container">
         ...
         <c:forEach var="pvo" items="${cartprolist}" varStatus="status">
            <tr id="titlehead2">
               ...
               <td class="info"><span><span id="proid${status.index}">${pvo.pcvo.fk_productid}</span><br />
                     <span id="proname${status.index}">${pvo.productname}</span>
                  ...
                  <form id="colorform${status.index}" method="post" action="ordercart.sg">
                     ...
                     <input type="hidden" value="${pvo.imgfilename}" id="imgpro${status.index}"/>
                     <select name="selectcolor" id="colorselec${status.index}"
                        ...
                        <c:forEach var="list" items="${colorlist.get(status.index)}">
                           <option value="${list}" class="opt">${list}</option>
                        </c:forEach>
                     </select>
                  </form></td>
               <td id="price${status.index}">${pvo.pcvo.price}</td>
               <td><input type="number" name="favnum"
                  id="numinput${status.index}" min="1" max="10" value="1" class="optcl"><br>
               <br> 
                  <button type="button" id="numchange${status.index}" class="numcl">변경</button>
               </td>
               <td id="oneresult${status.index}"></td>
               <input type="text" id="serialid${status.index}" value="${pvo.povo.productserialid}" hidden="true"/>
            </tr>
         </c:forEach>
      </table>
      <hr>
   ...
   <div>
      <span id="span3">선택상품을
         <button type="button" onClick="goCancel();" id="delbtn" >x 삭제하기</button>
      </span>
      <hr>
   </div>
   ...
   <div>
      <li id="finalbtn">
         <button type="button" onclick="allOrder" class="butt">전체상품주문</button>
         <button type="button" onclick="selectOrder();" class="butt">상품주문하기</button>
         <button type="button" class="butt">쇼핑계속하기</button>
      </li>
   </div>
   <br>
   <br>
