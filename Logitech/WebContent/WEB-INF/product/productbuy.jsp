...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$("span#paymoney").html(totalPrice);
		$("input#paymoney").val(totalPrice);
		...
		*/ 
		...
		//	alert(discount);
			...
			totalPrice = totalPrice - Number($("span#discountCost").text());
			$("span#paymoney").html(totalPrice);
			$("input#paymoney").val(totalPrice);
		...
		$.ajax({
			url:"<%= ctxPath%>/product/memberPoint.sg",
			data:{"memberno":"${sessionScope.loginuser.memberno}"},
			dataType:"JSON",
			success:function(json){
				var point = "0";
				
				if(json.totalPoint != null) {
					point = json.totalPoint;
				}
				
				$("span#totalPoint").html(point);
			},
		...
		$("input#point").blur(function(){
			...
			var totalPoint = Number($("span#totalPoint").html());
			
			if(point > totalPoint) {
				alert("보유하신 포인트를 초과할 수 없습니다.");
				return;
			}
			
			$("span#discountPoint").html(point);
			
			// 쿠폰 금액을 계산한 결제금액
			totalPrice = totalPrice - Number(point);
			$("span#paymoney").html(totalPrice);
			$("input#paymoney").val(totalPrice);
		});
	...
	function goPayment(){
		...
		var url = "<%= request.getContextPath()%>/product/payment.sg?userid="+userid+"&paymoney="+paymoney;
		window.open(url, "payment", "left=350px, top=100px, width=820px, height=600px");
		
		
	}
	
	function goPurchaseUpdate(userid, paymoney) {
		
		var frm = document.purchaseFrm;
		frm.userid.value = userid;
		frm.paymoney.value = paymoney;
		frm.action = "<%= request.getContextPath()%>/product/purchaseUpdate.sg";
		frm.method = "POST";
		frm.submit();
...
<div>
  ...
  <div class="form-group">
    <label for="inputdefault">받는 사람</label>
    <!-- <input class="form-control" id="inputdefault" type="text"> -->
  ...
  <div class="rightcolumn">
    ...
    <div class="card" style="width:300px;">
      <div class="fakeimg" style="height:100px;">
				  ...
				  <span>포인트 보유액</span>&nbsp;&nbsp;&nbsp;<span id="totalPoint"></span>
                  <br>
                  <label>사용할 포인트</label><input type="text" name="point" id="point" />
...
--%>  
	...
	<input id="originCost" type="hidden" value="totalPrice">
      ...
      --%>
 <%--     <c:if test="${eachcouponcode != null}"> --%>
      		<div class="fakeimg"><p>쿠폰할인: <span id ="discountCost"></span>원</p></div>
      		<div class="fakeimg"><p>포인트할인: <span id ="discountPoint"></span>원</p></div>
      		<div class="fakeimg"><h4 style="font-weight:bold;">결제금액&nbsp;:&nbsp;<span id="paymoney"></span>원</h4></div>
      	    <input id="totalCost" type="hidden" value="">
      	    <input id="paymoney" type="hidden" value="">
 <%--     </c:if> --%>
