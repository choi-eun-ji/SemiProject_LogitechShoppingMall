...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("tr#tHeadRow > th").click(function(){
			...
			location.href = '<%= ctxPath%>/manager/purchase/purchaseList.sg?currentPageNo=1&sizePerPage='+"${sizePerPage}"+'&searchOption1='+"${searchOption1}"+'&searchKey1='+"${searchKey1}"+'&searchOption2='+"${searchOption2}"+'&searchKey2='+"${searchKey2}"+'&selectedHead='+selectedHead+'&sortFlag='+sortFlag;
		});// end of $("tr.memberInfo").click(function(){})----------------------
		
		$("tr.pdetailInfo > td.purchaseRow").click(function(){
			rowCnt = $(this).parent().children().children(".rowCnt").text();
			// alert(rowCnt);
			fk_purchaseno = $(this).parent().children(".fk_purchaseno").text();
			...
			$.ajax({
				...
				success:function(json){
					...
					if(json.length > 0) {
						$.each(json, function(index, item){
							html += "<div style='background-color: white;'>" +
									"<div style='float: left; background-color: white; margin: 20px 100px;'><img src='"+item.imgfilename+"' style='width: 150px; height: 150px;' /></div>" +
									"<div style='float: left; width: 400px; background-color: white; margin: 20px 100px;'>" +
										"<ol>" +	
										"<li><span class='myli'>주문상세번호 : </span><span class='purchasedetailid' id='purchasedetailid"+index+"'>"+item.purchasedetailid+"</span></li>" +
										"<li><span class='myli'>제품판매아이디 : </span><span class='productserialid' id='productserialid"+index+"'>"+item.fk_productserialid+"</span></li>" +
										"<li><span class='myli'>주문량 : </span><span class='stock' id='stock"+index+"'>"+item.volume+"</span></li>" +
										"<li><span class='myli'>제품가격 : </span>"+item.price+"</li>";
										
							if(item.deliverystatus == "결제완료") {
								html += "<li><span class='myli'>상태 : </span><span class='deliverystatus' style='font-weight: bold; color: red;'>"+item.deliverystatus+"</span></li>" +
										"</ol>" +
										"</div>" +
										"<div style='float: right; background-color: white; margin: 85px 20px;'>" +
											"<label for='eachSelect"+index+"'>선택</label>&nbsp;&nbsp;<input type='checkbox' name='eachSelect' class='eachSelect' id='eachSelect"+index+"' />" +
										"</div>";
							} else {
								html += "<li><span class='myli'>상태 : </span><span class='deliverystatus' style='font-weight: bold; color: green;'>"+item.deliverystatus+"</span></li>" +
										"</ol>" +
										"</div>";
							}
										
							html +=	"<div style='clear: both; width: 100%; height: 3px; background-color: #b3b3b3;'></div>" +
									"</div>";
							
							// $.each 문 안에서 개별 상품의 배송 상태를 확인해 배송되지 않고
							// '결제완료'인 상품이 있을 경우 deliveryFlag 라는 변수를 true로 전환  
							if(item.deliverystatus == "결제완료") {
								deliveryFlag = true;
							}
						});
					...
					$("div#pdListInfo"+rowCnt).html(html);
				},
			...
			$("#checkAll").click(function(){
				...
				$("input[name=eachSelect]").each(function(){
					$("input[name=eachSelect]").prop("checked", bool);
				});
			})// end of $("#checkAll").click(function(){})-----------------------
			
			// @@ 전체선택과 개별선택의 체크박스를 일치 @@ //
			$(document).on("click", "input[name=eachSelect]", function(){
				var bFlag = false;
				
				$("input[name=eachSelect]").each(function(index){
					console.log($("input[id=eachSelect"+index+"]").prop("checked"));
					console.log(index);
					if(!$("input[id=eachSelect"+index+"]").prop("checked")) {
						bFlag = true;
						return;
					}
				});
				...
				if(bFlag) {
					// 체크박스 6개 중 1개라도 체크가 해제된 경우라면
					$("#checkAll").prop("checked", false);
				} else {
					// 체크박스 6개 모두가 체크가 된 경우라면
					$("#checkAll").prop("checked", true);
				}
				
			});// end of $(document).on("click", ".eachSelect", function(){})-------------------------
		...
		$("button#btnDelivery").click(function(){
			...
			$("input.eachSelect:checked").each(function(index){
				cnt = index + 1;
				var deliverystatus = $(this).parent().parent().find(".deliverystatus").text();
			...
			if(cnt == result) {
				alert("선택한 상품이 정상적으로 배송 처리되었습니다.");
			} else {
				alert("배송 처리에 실패하였습니다.");
			}
			/*
	...
	// 전
	function goPurchaseUpdate() {
		// alert(fk_purchaseno);
		
		$.ajax({
			url:"/Logitech/manager/purchase/purchaseListUpdate.sg",
			type:"POST",
			data:{"purchaseno":fk_purchaseno},
			success:function(){
				alert("해당 주문의 모든 상품 배송이 완료되었습니다.");
			},
			error: function(request, status, error){
				alert("code: "+request.status+"\n"+"message: "+request.responseText+"\n"+"error: "+error);
			}
			...
			<div class="col-sm-12">
				<div class="well divCell" style="background-color: white;">
					<select id="searchOption1" name="searchOption1">
						...
						<option value="fk_memberno">주문회원번호</option>
						<option value="payment">결제수단</option>
						<option value="purchaseday">주문일자</option>
						<option value="totalstatus">상태</option>
					</select>
			...
			<div class="col-sm-12">
				<div class="well divContent" style="background-color: white;">
					<table id="tblPurchase">
						...
						<tbody>
							<c:forEach var="pdvo" items="${pdList}" varStatus="vstatus">
								<tr class="pdetailInfo" id="pdetailInfo${vstatus.count}" data-toggle="collapse" data-target="#demo${vstatus.index}">
									<td class="fk_purchaseno purchaseRow">${pdvo.fk_purchaseno}</span></td>
									<td class="purchaseRow">${pdvo.purcvo.fk_memberno}<span class="rowCnt">${vstatus.count}</td>
							...
							<%--		<td><input type="checkbox" class="purchaseCheck" /></td> --%>
								...
								<!-- 인덱스를 활용해 각 주문 목록 바로 아래 상세 정보 collapse 띄우기-->
								<tr id="demo${vstatus.index}" class="collapse">
									<td colspan="8">
										<div class="bundle" style="background-color: white; margin: 20px;">
											<div class="pdListInfo" id="pdListInfo${vstatus.count}" style="background-color: white;"></div>
											<div style="float: right; background-color: white; margin: 20px;">
												<label for='checkAll'>전체선택</label>&nbsp;&nbsp;<input type="checkbox" id="checkAll" />
											</div>
											
											<div class="pdMember" style="clear: both; background-color: white;">
											</div>
											...
											<div id="deliveryResult" style="float: left; background-color: white; margin: 50px;"></div>
										</div>
									</td>
								</tr>
							</c:forEach>
