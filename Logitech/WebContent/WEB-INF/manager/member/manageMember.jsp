	...
	$(document).ready(function(){
		...
		$("#allCheck").click(function(){
			...
			$(".memberCheck").each(function(){
				$(".memberCheck").prop("checked", bool);
			});
		})// end of $("#allCheck").click(function(){})-----------------------
		
		// @@ 전체선택과 개별선택의 체크박스를 일치 @@ //
		$("input[name=memberCheck]").click(function(){
			var bFlag = false;
			
			$("input[name=memberCheck]").each(function(index){
				// console.log($("input[id=memberCheck"+index+"]").prop("checked"));
				// console.log(index);
				if(!$("input[id=memberCheck"+index+"]").prop("checked")) {
					bFlag = true;
					return;
				}
			});
		...
		$("#dropout").click(function(){
			// 선택한 체크박스의 회원 탈퇴 처리를 일괄 처리
			$("input.memberCheck:checked").each(function(){
				var indexVal = $(this).val();
				var userid = $("td#userid"+indexVal).text();
				
				$.ajax({
					url:"<%= ctxPath%>/manager/member/memberDropout.sg",
					data:{"userid":userid},
					type:"POST",
					success:function(){
			...
			<div class="col-sm-12">
				<div class="well divContent" style="background-color: white;">
					<table id="tblMember">
						...
						<tbody>
							<c:forEach var="mvo" items="${memberList}" varStatus="vstatus">
								<tr class="memberInfo">
									<td class="memberno memberRow">${mvo.memberno}</td>
									<td class="memberRow userid" id="userid${vstatus.index}">${mvo.userid}</td>
									...
									<td><input type="checkbox" name="memberCheck" class="memberCheck" id="memberCheck" value="${vstatus.index}" /></td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
					...
					<div id="divBar" style="background-color: white; margin: 30px;">
						...
						<div style="float: right;">
							<button type="button" id="dropout" style="border: none; width: 150px; height: 50px; background-color: #1a1a1a; color: white; text-align: center;">탈퇴 처리</button>
