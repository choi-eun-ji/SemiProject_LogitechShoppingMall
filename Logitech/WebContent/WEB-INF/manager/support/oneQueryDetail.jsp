...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("button#oqAnswerComplete").click(function(){
			alert("");
			$.ajax({
				url:"<%= ctxPath%>/manager/support/oneQueryAnswer.sg",
				type:"POST",
				data:{"seq_oq":"${oqvo.seq_oq}",
...
<form name="queryAnswerFrm">
	<div id="oneQueryState">
		<div>
			<div class="row">
				<div class="col-sm-12">
					<div class="divContent">
						<c:if test="${empty oqvo}">
						존재하지 않는 문의입니다.<br>
					</c:if>
					
					<c:if test="${not empty oqvo}">
						<div id="oneQueryInfo">
							<ol>
								<li style="display: none;">${oqvo}</li>	
								<li><span class="myli">문의 번호:&nbsp;&nbsp;${oqvo.seq_oq}</span></li>
								<li><span class="myli">회원아이디:&nbsp;&nbsp;<a href='#' data-toggle='popover' title="이름 : ${mbvo.name}" data-content="이메일 수신 동의 : ${mbvo.agreeemail}<br>문자 수신 동의 : ${mbvo.agreesms}" data-html="true">${oqvo.mvo.userid}</a></span></li>
								<li><span class="myli">문의 카테고리:&nbsp;&nbsp;${oqvo.category}</span></li>
								<li><span class="myli">제목:&nbsp;&nbsp;${oqvo.title}</span></li>
								<li><span class="myli">내용:&nbsp;&nbsp;${oqvo.content}</span></li>
							</ol>
							<input type="hidden" id="hiddenSeq_oq" name="seq_oq" value="${oqvo.seq_oq}" />
